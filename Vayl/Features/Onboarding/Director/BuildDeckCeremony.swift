...
final class BuildDeckCeremony {
    ...
    /// Residual strain floor — every reseal is imperfect, so the lattice never
    /// unwinds below this after a strike. The visible proof the user is winning.
    var stressFloor: Double { Self.stressFloors[min(tapCount, 3)] }
    private static let stressFloors: [Double] = [0, 0.15, 0.35, 1.0]

    /// Strain pulse level (0…1) — how agitated the contained energy is between
    /// strikes; drives the core glow's breathe rate + depth in the case view.
    var strainPulse: Double { Double(min(tapCount, 3)) / 3 }
