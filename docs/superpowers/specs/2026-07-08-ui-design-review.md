...
# PASS 2 — GAP & EDGE CASE AUDIT

# Subagent G — Gap & Edge Case Audit

## Pass 2 — Gap & Edge Case Audit (Phase G)

Method note: full Pass 1 document read (all lines); every finding below was verified in code at the cited line. Three parallel sweeps (laundered token values, one-shot motion, coherence raw material) plus direct inspection of every never-critiqued state. Nothing below duplicates a Pass 1 item; where a Pass 1 item is adjacent, the relationship is stated.

### G.1 States inventoried but never critiqued

1. **Sign-In legal sheet / legal footnote** — Finding [contrast + hierarchy]: Before: the legal consent line ("By continuing you agree to our Terms & Privacy Policy") renders in `AppColors.textMuted` (1.76:1) at `AppFonts.meta` (10pt) with links tinted `textSecondary` (SignInView.swift:102-105) — load-bearing legal copy below every floor Phase 6 set, and tap targets at 10pt → After: `AppFonts.caption` + `textTertiary` body with `.tint(AppColors.textAccent)` links. (The raw `.sheet` at :124 is already punch-listed P2; this is the copy itself, which Phase 6's 28-site `textMuted` list missed.)
2. **Session Builder trim/restore interaction** — Finding [motion]: trim → "Set aside tonight" and restore-back mutate `store.entries`/`store.trimmed` with **no `withAnimation` anywhere in view or store** (SessionBuilderView.swift:157-187, 201-216; SessionBuilderStore.swift:114, 121 — verified zero `withAnimation` hits in both files). A card leaving the lineup for the set-aside shelf is a spatially meaningful move; today it jump-cuts → wrap the store mutations at the call sites in `withAnimation(AppAnimation.standard)` so the List animates the row move. Second finding [a11y]: the per-card timer chip and the whole-session budget chip are blind-cycle controls (tap advances through nil/5m/…, SessionBuilderView.swift:157-168, 263-279) with no `accessibilityValue` and no hint that tapping cycles → add `.accessibilityValue(timerLabel(...))` + `.accessibilityHint("Cycles the timer")`.
3. **Settings destructive confirms** (SettingsView.swift:100-134) — Inspected — no finding. System `confirmationDialog`/`alert` is the right grammar for destructive confirms; the delete copy ("they keep their own data and return to being unpaired. This cannot be undone.") is the app's honesty ethic done right. The dialog-vs-alert split (unlink/sign-out = dialog, delete = alert) tracks severity and is defensible.
4. **Session close reflection sheet states** — Three findings. [a11y, P0-class]: `ReflectionSlider` is a bare `DragGesture` (SessionCloseView.swift:359-364) with no `accessibilityValue`/`accessibilityAdjustableAction` — a VoiceOver user cannot answer "who carried it" or "did you feel heard"; and `FlowChips` word-pills are `.onTapGesture` Texts (:255) with no button trait or selected state — the whole reflection is inoperable under VO (the app has exactly 2 `accessibilityAdjustableAction` sites; this needs to be the third). [haptic drift]: chip selection fires `UISelectionFeedbackGenerator().selectionChanged()` (:140) — the only selection-changed haptic on a chip toggle in the app (equivalent chips elsewhere use `.impact(.light)`). [color]: the slider knob rebuilds the cyan→magenta gradient inline (:350-352) — a new site for the Pass 1 "inline spectrum gradients" item.
5. **Pulse uncharted resolution beats** — Inspected — no new finding. Phase 4 §8 already judged the sequence (positively); the `Task.sleep(320ms/1.5s)` literals are already in the Phase 3 cat-4 sweep. The revisit-cancels-drift guard (PulseCheckInView.swift:363-367) is correct.
6. **Confirmation credential editor (CredentialEditorSheet + CredentialEditorOverlay)** — Findings: [restraint] the hand-rolled grab handle is a 40×4pt capsule **filled with the full spectrum gradient** (CredentialEditorSheet.swift:44-47) — a seventh gradient-on-tiny site for Phase 5's list (three stops in 4pt of height), and it diverges from the standard `.vaylSheet` grabber → neutral `AppColors.borderDefault` capsule. [color] the overlay scrim is raw `Color.black.opacity(0.1)` (CredentialEditorSheet.swift:333) — no whisper-scrim token exists; mint one beside `scrimHeavy`. [rhythm] dismiss threshold is a bare `120` (:352). (The three unhaptic `.onTapGesture` rows are already in Phase 3 cat 12.)
7. **Play joiner banner vs Home's** — Inspected — no drift: both wrap `PendingSessionBanner` identically (PlayView.swift:114-128; HomeDashboardView.swift:635-651 — same paddings, same transition, same spring). Their shared `.move(edge: .top)` exit is a new violation, reported once in G.4.
8. **Deck-engaged tab-bar recede** — Finding [system]: the recede treatment is the magic pair `opacity 0.25 / blur 6`, hand-duplicated **six times across two files** (AppShell.swift:61-62; HomeDashboardView.swift:227-228, 242-243, 534-535, 551-552) with a comment in AppShell admitting it "matches the greeting/getting-started fade in HomeDashboardView" — coupled by convention only → one `.vaylRecede(_ engaged: Bool)` modifier (or `AppAnimation.recedeOpacity`/`recedeBlur` tokens) consumed by both files.
9. **Vault EventEntryEditor** — Finding [component]: the Save CTA is a **third primary-CTA voice** — a solid `AppColors.accentSecondary` capsule with `.white` text (EventEntryEditor.swift:143-148) — distinct from both VaylButton and the Session cover's spectrum capsules; Phase 8's CTA-consolidation item lists neither this site nor this variant → add to that item; disabled state is raw `.opacity(0.5)` (:152).
10. **DiscussionCardView** — Inspected — no finding. Clean tokens, reuses ConversationCard as documented; its `tracking(1.0)` folds into the existing tracking-sprawl item; its void-only background is captured in the sheet-background axis (G.5 #8).
11. **Reflection full-pill sheet** (ReflectionBannerView.swift:284-366) — Finding [presentation grammar]: the sheet interior is a `NavigationStack` with `.navigationTitle("How did that land?")` + inline system nav bar (:285, 363-364) — **the only system navigation bar inside any sheet in the app**, against the grammar every other sheet follows (in-content title, no NavigationStack; the orphaned PairingSettingsView was flagged for exactly this and deleted) → drop the NavigationStack, title in-content at the sheet-title tier. Also: its Done CTA is yet another bespoke fill (`ctaFill` computed with `colorScheme` branches, :345-356) and the background is `pageBackground` (:362) — see G.5 #8.
12. **PulseInfoSheet** — Finding [dead affordance, P0-class]: the sheet is a shipped stub — its entire body is `AppColors.void` + the title "About the Pulse" in `textMuted` (HomePulseRail.swift:209-218), presented live from the dashboard (HomeDashboardView.swift:502, at heightFraction 0.5). A user who taps the pulse-rail info affordance gets a half-screen of nothing → either write the two-paragraph explainer (the check-in flow's own copy already explains the field) or remove the entry point until it exists. Pass 1's dead-affordance sweep (fake search, empty About rows, dead quiz CTA) missed this one entirely.
13. **SessionSettingsSheet** — Two findings. [a11y]: option rows hide the checkmark from VoiceOver (`accessibilityHidden(true)`, SessionSettingsSheet.swift:111) and never add `.isSelected` — a VO user cannot tell which reader/length is chosen. Same gap in SettingsIdentityView.swift:181-183, while **SettingsCompositionView.swift:49 does it correctly** (`.accessibilityAddTraits(store.composition == option ? .isSelected : [])`) — copy the in-house correct pattern to both. [coherence]: the selected checkmark is `spectrumCyan` here (:110) but `accentPrimary` in CredentialEditorSheet (:234, :264) — the same control in two colors (G.5 #9). Micro: "Short  ~10 min" uses double-spaces as a layout separator (:78-80) → real HStack spacing.
14. **FindingDetailView interior** — Findings: [contrast] the hero stat renders `AppFonts.displayHero` in solid `AppColors.spectrumPurple` (FindingDetailView.swift:24) — 3.15:1 on void and 2.96:1 on cards per Phase 6's own table, borderline-failing even as large text on the `modalBackground` this sheet uses → `spectrumText` gradient anchored on cyan, or `textPrimary` with a purple type-chip. [coherence] in-file overline casing split: "One honest limitation" (sentence case, :37) vs "CONNECTED RESEARCH" (caps literal, :47) on the same screen. [honesty] the file's own header says "STUB layout" (:4) — either finish it or strip the comment; the layout is in production.
15. **ResourcesOverlayView interior** — Inspected — no finding. The lifeguard copy, cyan/gold tier split matching the safety-accent system, live tel:/sms: actions, and `arrow.up.right` (external) vs `arrow.right` (call) semantics are all right. Raw `.system(size: 19)` already in the Phase 3 sweep.

### G.2 Surfaces missing from the Phase 1 list

1. **SingleGreetingOverlay** — `Vayl/Features/Onboarding/Components/SingleGreetingSheet.swift`, presented live by OnboardingCanvasView.swift:219-224 when a user confirms "I'm single" in ContextPhase. This is the product's single most principle-loaded surface — the honest off-ramp CLAUDE.md demands — and it was never inventoried, critiqued, or accessibility-audited. Assessment: strong (full tap contract with 0.96/haptic, tokens, wayfinding bullets, honest copy). Two nits: raw `Color.black.opacity(0.6)` scrim (:17 — same pattern as the two `scrimHeavy` punch sites; add as a third), and its outline-capsule "Got it" is one more CTA variant for the consolidation item.
2. **SettingsCard** — `Vayl/Design/Components/Cards/SettingsCard.swift`, 16 call sites across all seven Settings screens. Phase 5's "three card-chrome systems" axis missed it: it wraps `vaylGlassCard` but adds an inline-rebuilt spectrum hairline (cyan/purple/magenta at raw 0.28/0.26/0.28) on **every** Settings card — the signature on 16 utility cards at once, against Phase 4's own one-accent restraint rulings, and rebuilt inline instead of using the existing `SpectrumHairline` component. Assessment: consolidate onto `SpectrumHairline` and decide whether Settings cards carry the signature at all (recommend: no; `borderSubtle` hairline, signature stays on the name header).
3. **CompositionConfirmCard** — `Vayl/Features/Pairing/CompositionConfirmCard.swift`, shown on the pairing linked screen (PairingInviteView/PairingJoinView). Assessment: exemplary — self-documents the discovery-tool bright line ("wayfinding… never a statement about either person") and uses VaylButton. No finding.
4. **SessionTimerBar** — `Vayl/Features/Sessions/Components/SessionTimerBar.swift`, in SessionPlayerView. A whole state family (timer running → elapsed "no rush, wrap up when you're ready") absent from the Phase 2 state table. Assessment: on-voice, tokened; no finding.
5. **ContextKickerView / CardBackFlipView** — session-player interiors (SessionPlayerView call sites); CardBackFlipView's asymmetric transition is the model-correct opacity-removal pattern. No findings; listed for inventory completeness.
6. **FounderLetterSheet** — reached Phases 6/8 (fixed 420pt frame) but never entered the Phase 1 surface list, so it got an accessibility fix without a taste pass. Assessment: no additional finding on inspection of its Phase 6 citations.
7. **DesireMatchDetail** — shared card body inside DesireStarDetailSheet/DesireMapListSheet. Verified NOT a dead affordance: "Talk about this →" routes to the Vault (DesireRevealView.swift:369, 385) and "Explore in Learn" hides itself while its handler is nil (DesireMatchDetail.swift:65) — honest stub handling worth naming as the pattern ResearchDatabaseView should have used.
8. **Orphans to add to the Pass 1 orphan list** (MeCardSheet/MeCardCompact/PrismView): **SyncMatch** (`Features/Sessions/Components/SyncMatch.swift`) — zero external call sites. Same delete-or-land decision.
9. **Widgets/App Intents**: no widget or intents target exists in the project despite `AppColors.widgetBackground` shipping as a token — the token is speculative infrastructure; note for the token-hygiene pass. Also `Design/Components/Navigation/VaylSheet.swift` (the actual chrome behind every sheet, with its `.full`-vs-`.hairline` signature knob reserved for the paywall) was never documented by Pass 1 — the signature-assignment rule lives only in that file's comment.

### G.3 Token violations grep missed

Strongest laundered values (all verified in context; none in Pass 1's lists):

1. `Features/Home/Components/HomeWidgetShell.swift:179` — `private let corner: CGFloat = 20` driving clip + stroke (:192, :195) — exact duplicate of `AppRadius.container`.
2. `Design/Components/Cards/CardLayout.swift:31` — `static let cornerRadius: CGFloat = 20` — the app-wide card corner as a raw literal in a component enum; every card face inherits the divergence → `AppRadius.container`.
3. `Features/Learn/Views/LearnCardStyle.swift:36` — `private let fillOpacity: Double = 0.04` inside the app's de-facto default ButtonStyle (`PressableCardStyle` home) — the one raw value in the most-reused style in the app.
4. `Design/Components/Navigation/VaylSheet.swift:38,41` — `purpleTint = 0.13`, `darken = 0.18` — raw washes in the shared chrome behind **every** sheet.
5. `Design/Components/Cards/CardFaces/ContextCardFace.swift:111-113` — `turnDur = 0.6`, `dropDur = 0.95`, `liftDur = 0.45` — three choreography durations named-constant-laundered past the duration grep.
6. `Features/Home/Components/HomeLexicon.swift:64` — `slideDuration: Double = 0.55` (pager slide).
7. `Features/Onboarding/Renders/AnimatedSignature.swift:36-37, 46` — `glowBlur = 6`, `glowOpacity = 0.6`, `middleDuration = 0.55` — glow spec outside AppGlows + raw duration.
8. `Design/Components/Buttons/SelectablePill.swift:24-25` — default params `height: CGFloat = 46`, `fontSize: CGFloat = 15` fed into `AppFonts.body(fontSize,…)` — a raw font size invisible to any `.font(.system` grep.
9. `Design/Components/Cards/InfiniteCarousel.swift:21` — `var interval: TimeInterval = 5` — the Learn ticker cadence Pass 1 flagged is baked in as the component's **default**; fixing the two call sites without fixing the default leaves the trap armed → default to the new `ambientDwell`.
10. `Features/Desire Map/Views/Components/DesireStarView.swift:151-152` — computed properties returning raw values (`entranceStartScale { 0.2 }`, `seedRestOpacity { 0.6 }`) — the exact computed-literal pattern.
11. `Features/Onboarding/Phases/StatPhase.swift:339` — `var fontSize: CGFloat = 140` default into `AppFonts.statHero`.
12. Raw scrims the color grep missed: `CredentialEditorSheet.swift:333` (`black.opacity(0.1)`), `SingleGreetingSheet.swift:17` (`black.opacity(0.6)`) — add to the `scrimHeavy` punch item; EventEntryEditor.swift:124 chip `.white` (sibling of the already-cited :145).
Also verified but deliberately down-ranked (author-annotated FEEL-GATE device physics): MapView.swift:32-47, SessionPlayerView.swift:30-31, HomePulseRail.swift:31/36, GenderPhase.swift:305 — laundered but sanctioned; fold into sweeps only if the human de-ratifies FEEL-GATE.

### G.4 One-shot motion violations

1. **Banner exits slide (exit must be opacity-only)** — `.transition(.move(edge: .top).combined(with: .opacity))` used symmetrically, so removal also travels: PlayView.swift:126 and HomeDashboardView.swift:647 (PendingSessionBanner), plus ReflectionBannerView's host with the removal move **spelled out explicitly** in the asymmetric pair (HomeDashboardView.swift:621-625). All driven by `AppAnimation.spring` → insertion may keep the quiet slide (≤16pt) or move to `.vaylDepth(.quiet)`; removal becomes `.opacity` on `AppAnimation.exit`.
2. **Lens (Me/Us) swaps bypass the staple** — `withAnimation(AppAnimation.spring)` drives the layer content swap at MapView.swift:221/:234 and PulseFullView.swift:85/:97 with bare `.transition(.opacity)` on the content — a content handoff on a reactive spring (settle > the 0.55s quiet ceiling) instead of `depthQuiet` → `.vaylDepth(.quiet)` + `.animation(AppAnimation.depthQuiet, value: store.layer)`.
3. **Vault segment swap is a hard cut** — `switch store.segment` (VaultSheet.swift:85) has no transition and no `withAnimation` at the segmented control; Desire Map ⇄ Agreements ⇄ Log jump-cuts inside a glass sheet → same `.vaylDepth(.quiet)` treatment (LearnSegmented's 0.3s pill highlight is compliant; it's the content region that has nothing).
4. **Session Builder trim/restore jump cut** — see G.1 #2 (a one-shot motion gap, cross-referenced not double-counted).
Checked and clean: no one-shot durations > 1.2s outside ceremonies; no competing animations on one property (the stacked `.animation(_, value:)` groups each target distinct values); no press-scale values other than 0.96; DesireRevealView/DesireMapView `.move`/asymmetric-scale transitions are the documented `desireSheetRise`/`desireDepthExit` token intents; VaylPresentation's internal `.move` is the sanctioned arrive primitive; OB-canvas slides exempt.

### G.5 Coherence issues Phase 5 missed

1. **CTA label casing** — "Try Again" (Title Case: PairingInviteView.swift:282, PairingJoinView.swift:239, HomeRouterView.swift:207) vs "Try again" (sentence: PlayView.swift:190, PlayEmptyState.swift:40) — the same recovery action in two casings. House style is otherwise sentence case ("Unlock everything", "Take your deck", "Generate new code") → rule: sentence case everywhere; the three Title Case sites conform.
2. **Register drift inside the Session cover** — the cover speaks lowercase ("we're ready" AirlockView.swift:152, "hold to deal" SessionPlayerView.swift:440) *and* sentence case ("Cancel session" / "Not now" SessionLobbyView.swift:80) with no rule for which voice a session CTA takes → write it down: inside the cover, ritual actions are lowercase, escape/utility actions are sentence case (that is the emerging pattern; make it deliberate).
3. **Uppercasing mechanism split** — some overlines uppercase via `.textCase(.uppercase)` (33 sites), others via pre-uppercased string literals ("CONNECTED RESEARCH", "ADD A NOTE", "AN HONEST MOMENT", "WHO READS FIRST", "STATUS"…) — the literal path silently skips whatever tracking convention the modifier path carries, and produces in-file splits like FindingDetailView (G.1 #14) → the planned `overlineTracked` token (Phase 8 P1) must bake in `.textCase(.uppercase)` so source strings stay sentence case.
4. **Haptic grammar has no assignment rule and two idioms** — declarative `.sensoryFeedback` (~30 sites) and imperative `UIImpactFeedbackGenerator` (~50 sites) coexist; equivalent actions drift: seal/commit is `.rigid` in the four reveal components but `.medium` across OB commits and `.success` in HoldToLockInRing/GenderPhase; "open a protected surface" is `.medium` at VaultDoorCard.swift:30 and MapView.swift:300 but `.light` at MapPulseHero (field cover) — same mental weight, three intensities → one written scale (light = select/navigate, medium = commit, rigid = two-device seal, heavy = safe word only, success = terminal completions) and standardize new code on `.sensoryFeedback`.
5. **Relative-date formatting: three implementations, one of them lying** — HomePulseRail's `relativeTime` returns "yesterday" for **any** age ≥ 24h (HomePulseRail.swift:198-204 — a 6-day-old check-in reads "yesterday", directly undercutting the stale-honesty discipline Phase 4 praised); PulseStore.swift:66-74 hand-rolls the same job correctly ("N days ago"); PairingInviteView.swift:71 uses system `.formatted(.relative(presentation: .named))` → one shared relative-date helper (system formatter), which fixes the bug by deletion.
6. **Icon system shipped-but-orphaned** — `AppIcons` tokens exist (AppEnums.swift:263-274) with exactly **one** adopter (PairingSettingsView.swift:190 — a file Pass 1 slated for deletion, which would take adoption to zero); everywhere else chevrons are raw `Image(systemName:)` and some CTAs use text glyphs instead of symbols ("→" in HomeDashboardView.swift:658 and DesireMatchDetail.swift:55, "›" in DesireMapView.swift:889). Lock glyph splits `lock.fill` (6 sites) vs `lock` (SessionBuilderView.swift:172, VaultDesireSection.swift:147) → adopt AppIcons or delete it; pick one lock variant.
7. **Hairline treatment: five dialects** — Settings: `Divider().overlay(borderSubtle)` (12 sites); Vault/Map: `Rectangle().fill(borderSubtle).frame(height: 1)` (7 sites); OB phases: sub-pixel `0.5`/`0.75` heights in spectrum colors; PartnerChipExpand.swift:96: a `spectrumText.opacity(0.35)` divider; PrismView: bare uncolored `Divider()` → one `VaylHairline` primitive (the `SpectrumHairline` component already exists for the signature variant).
8. **Sheet interior backgrounds: four different surfaces** — void + atmosphere (SessionSettingsSheet), `modalBackground` (FindingDetailView.swift:60, ResourcesOverlayView.swift:32), bare void (DiscussionCardView.swift:44), `pageBackground` (ReflectionBannerView.swift:362) — all under the same VaylSheet chrome, so sheet depth reads differently per feature → rule: sheets ride the chrome's own `modalBackground` wash; only covers get void + atmosphere.
9. **Selection-checkmark color split** — `spectrumCyan` (SessionSettingsSheet.swift:110) vs `accentPrimary` (CredentialEditorSheet.swift:234/:264) for the identical `checkmark.circle.fill` selected-row control → `accentPrimary` (per the Phase 8 spectrum rule's small-indicator clause).
10. **`.vaylSheet` heightFraction sprawl** — eight fractions in use (0.4/0.5/0.55/0.66/0.75/0.85/0.9/0.92) with 0.9 and 0.92 both meaning "tall task sheet", and the `screenHeight:` parameter passed by only three files, so equal-fraction sheets can resolve different heights (MapView passes it; VaultSheet 0.9 and HomeRouterView 0.92 don't) → three named detents in VaylPresentation (`.compact` 0.5, `.standard` 0.66, `.tall` 0.92) and resolve screenHeight internally.
11. **Placeholder voice** — Sessions placeholders are lowercase-with-ellipsis ("type it, then seal", "a line for future-you about tonight…") while Vault/Settings/OB are sentence case ("Your name", "Propose an agreement", "A date, a night, a moment") — consistent with #2's register rule if written down; "optional" appears as a bare lowercase placeholder twice in EventEntryEditor (:64, :69).

### G.6 Phase 8 items needing concretion

1. **P1 [taste] "design a quiet degraded-connectivity chrome for the player"** — not executable as written. Concretion: `SessionSyncCoordinator` already publishes `isConnected` (SessionSyncCoordinator.swift:33). In SessionPlayerView's counter line ("N cards left", :137-143), when `!isConnected` swap the counter for "reconnecting…" (`AppFonts.caption`, `textTertiary`, `.transition(.opacity)` on `AppAnimation.enter`/`exit`) and disable the seal/commit control while disconnected. No banner, no red, nothing modal.
2. **P2 "mint an opacity token ramp"** — no scale proposed. Concretion from the observed clusters: `AppOpacity.whisper = 0.04` (glass lifts: LearnCardStyle, whisperFill), `.hairline = 0.08` (tracks/inactive fills), `.border = 0.15` (rims, = borderActive), `.dim = 0.25` (recede/idle), `.stroke = 0.45` (accent outlines), `.glowFloor = 0.3` / `.glowPeak = 0.7` (the mandated breathe band). Seven names cover ~80% of the 661 literals.
3. **P1 "One primary-CTA voice — VaylButton everywhere (or bless one named capsule component)"** — the either/or is already decided by the codebase: VaylButton ships `size: .compact` (used in SessionSettingsSheet.swift:43) and `style:`/`.fullWidth` (used in DesireMatchDetail.swift:55-59). Concretion: rule = full-width commits are `VaylButton(.fullWidth)`, in-flow seals/pills are `VaylButton(size: .compact)`; delete the bespoke capsules (add EventEntryEditor.swift:143-148 and ReflectionBannerView.swift:345-356 to the item's site list — two variants Pass 1 missed).
4. **P1 card-chrome consolidation "converge on the two canonical surfaces (or bless a tokenized HomeWidgetShell)"** — premise is off: there are four chromes (themedCard, vaylGlassCard, HomeWidgetShell, and the unreviewed SettingsCard). Concretion: `vaylGlassCard` is the base; SettingsCard becomes `vaylGlassCard` + shared `SpectrumHairline` (or drops the hairline per G.2 #2); HomeWidgetShell is rebuilt on `vaylGlassCard` + `AppElevation.cardShadow` + AppGlows, keeping only its orb layer as custom.
5. **P0 Learn dead quiz CTA "gate or remove the CTA"** — decision left open. Concretion: remove the `QuizCarouselSection` from LearnView's stack (LearnView.swift:19-27) behind the same DEBUG pattern as couch mode until a runner ships; Research + Content Hub carry the tab. One-line change, honest surface.
6. **P1 "Breathing-tempo rule — document the aura-vs-chrome tempo assignment"** — the rule to write: living/biological surfaces (auras, orbs, anything representing a person) breathe at `auraBreathe` (5.4s); inert chrome (pills, dots, borders, waiting indicators) pulses at `ambientPulse` (2s); there is no third tempo and no arithmetic on either.
7. **P1 empty-state "adopt (or extract) the shared component"** — concretion: move `MapEmptyState` (MapPrimitives.swift:43-66) to `Design/Components/VaylEmptyState.swift` with `(icon:, headline:, sub:, cta: (String, () -> Void)? = nil)`; keep `typealias MapEmptyState = VaylEmptyState` so Map/Vault call sites don't churn.
8. **P2 design-layer raw colors "promote to VaylPrimitives or named local render constants"** — decision rule: a color used by ≥2 files goes to VaylPrimitives (tier 1); single-file effect internals become a file-private `enum Palette` with a doc comment; `VaylAppIcon` is exempted outright (brand-asset renderer, not UI).
9. **P2 sheet interior margins "pick one and write it down"** — pick `lg` (24): it matches the documented screen-edge margin and the majority (Vault, Settings, DesireMapListSheet); migrate SessionBuilderView and ResearchDatabaseView from `md`.

### Net-new punch items

**P0**
- [taste] HomePulseRail.swift:209-218 — PulseInfoSheet is a shipped stub (title-only half-sheet presented live from HomeDashboardView.swift:502) → write the explainer content or remove the entry point until it exists. (G.1 #12)
- [system fix] HomePulseRail.swift:198-204 — `relativeTime` reports "yesterday" for any check-in ≥ 24h old, fabricating recency on the surface whose stale-honesty Pass 1 praised → replace all three relative-date implementations with one system `.formatted(.relative(presentation: .named))` helper. (G.5 #5)
- [system fix] SessionCloseView.swift:255, 359-364 — the session reflection is inoperable under VoiceOver: ReflectionSlider has no accessibility value/adjustable action and FlowChips are trait-less `.onTapGesture` Texts → `.accessibilityElement()` + label/value + `accessibilityAdjustableAction` on the sliders; Button-ify the chips with `.isSelected`. (G.1 #4)

**P1**
- [system fix] SessionSettingsSheet.swift:111 + SettingsIdentityView.swift:181-183 — selected option invisible to VoiceOver (checkmark hidden, no `.isSelected` trait); copy the correct in-house pattern from SettingsCompositionView.swift:49. (G.1 #13)
- [system fix] PlayView.swift:126, HomeDashboardView.swift:621-625 + :647 — banner removal transitions slide (exit must be opacity-only) → removal `.opacity` on `AppAnimation.exit`. (G.4 #1)
- [system fix] MapView.swift:221/:234 + PulseFullView.swift:85/:97 — Me/Us lens content swaps ride `AppAnimation.spring` instead of the depth staple → `.vaylDepth(.quiet)` + `depthQuiet`. (G.4 #2)
- [system fix] VaultSheet.swift:85 — segment content switches with no transition (hard cut) → `.vaylDepth(.quiet)` on the segment region. (G.4 #3)
- [system fix] SessionBuilderView.swift:157-216 — trim/restore/reorder mutations unanimated (jump cut) → `withAnimation(AppAnimation.standard)` at the mutation call sites; add `accessibilityValue`/hint to the two blind-cycling timer chips. (G.1 #2)
- [system fix] ReflectionBannerView.swift:285, 362-364 — the only NavigationStack + system nav title inside a sheet, on `pageBackground` → in-content title at the sheet-title tier, `modalBackground`, drop the stack. (G.1 #11)
- [system fix] SignInView.swift:102-105 — legal consent copy at `textMuted` (1.76:1) / 10pt with 10pt link targets → `caption`/`textTertiary` + `textAccent` links; add to the textMuted-retirement list. (G.1 #1)
- [system fix] AppShell.swift:61-62 + HomeDashboardView.swift:227-228/242-243/534-535/551-552 — the deck-engaged recede pair (opacity 0.25 / blur 6) hand-duplicated across six sites in two files → one `.vaylRecede(_:)` modifier / token pair. (G.1 #8)
- [system fix] Design/Components/Cards/SettingsCard.swift:16-27 — inline-rebuilt spectrum hairline at raw opacities on all 16 Settings cards → `SpectrumHairline` component, and demote Settings cards to `borderSubtle` so the signature isn't wallpaper. (G.2 #2)
- [system fix] Haptic grammar — two idioms and drifting weights on equivalent actions (seal `.rigid` vs commit `.medium` vs `.success`; open-protected `.medium` vs `.light`) → write the five-step scale (G.5 #4) into AppAnimation/CLAUDE.md and standardize on `.sensoryFeedback`.
- [system fix] VaylPresentation — replace the eight ad-hoc `heightFraction` values with three named detents (`.compact` 0.5 / `.standard` 0.66 / `.tall` 0.92) and resolve `screenHeight` internally (0.9-vs-0.92 and the inconsistently-passed parameter both disappear). (G.5 #10)
- [system fix] Laundered token constants — HomeWidgetShell.swift:179 and CardLayout.swift:31 (`20` → `AppRadius.container`), LearnCardStyle.swift:36, VaylSheet.swift:38/41, ContextCardFace.swift:111-113, HomeLexicon.swift:64, AnimatedSignature.swift:36-46, SelectablePill.swift:24-25, InfiniteCarousel.swift:21 (default → `ambientDwell`), DesireStarView.swift:151-152, StatPhase.swift:339 → tokens; add CredentialEditorSheet.swift:333 + SingleGreetingSheet.swift:17 scrims to the scrim item. (G.3)
- [taste] CredentialEditorSheet.swift:44-47 — spectrum-gradient 4pt grab handle (seventh gradient-on-tiny site) → neutral `borderDefault` grabber matching `.vaylSheet`. (G.1 #6)
- [taste] FindingDetailView.swift:24 — 64pt hero stat in solid `spectrumPurple` (≈3:1, borderline even as large text) → `spectrumText` anchored on cyan or `textPrimary`. (G.1 #14)

**P2**
- [system fix] CTA casing rule — sentence case everywhere; conform "Try Again" ×3 (PairingInviteView.swift:282, PairingJoinView.swift:239, HomeRouterView.swift:207); write the cover's lowercase-ritual/sentence-utility split down. (G.5 #1-2)
- [system fix] `overlineTracked` must bake in `.textCase(.uppercase)`; convert the pre-uppercased literals (FindingDetailView.swift:47, ReflectionBannerView.swift:289-302, SingleGreetingSheet.swift:30, SessionSettingsSheet.swift:66/77, PairingSettingsView…) to sentence-case sources. (G.5 #3)
- [system fix] One `VaylHairline` primitive to replace the five divider dialects (Settings `Divider().overlay`, Vault `Rectangle()` hairlines, OB sub-pixel variants, PartnerChipExpand.swift:96 spectrum divider). (G.5 #7)
- [system fix] Sheet-background rule — sheets use the chrome's `modalBackground`; migrate SessionSettingsSheet (void+atmosphere), DiscussionCardView (bare void), ReflectionBannerView (pageBackground). (G.5 #8)
- [system fix] AppIcons: adopt or delete (one adopter, inside a file slated for deletion); unify `lock` vs `lock.fill`; replace text-glyph arrows in CTA labels (HomeDashboardView.swift:658, DesireMatchDetail.swift:55, DesireMapView.swift:889) with SF symbols or plain labels. (G.5 #6)
- [system fix] Selection checkmark color — `accentPrimary` everywhere (SessionSettingsSheet.swift:110 conforms to CredentialEditorSheet). (G.5 #9)
- [system fix] SessionCloseView.swift:140 — lone `UISelectionFeedbackGenerator` on a chip toggle → `.impact(.light)` per the haptic scale. (G.1 #4)
- [system fix] Features/Sessions/Components/SyncMatch.swift — orphaned (zero call sites) → add to the delete-or-land list beside MeCardSheet/PrismView. (G.2 #8)
- [taste] EventEntryEditor.swift:143-152 — third CTA voice (solid accentSecondary capsule) + raw disabled opacity → `VaylButton(.fullWidth, isDisabled:)`. (G.1 #9)
- [taste] SingleGreetingOverlay — add to the Phase 1 surface list and the scrim item; otherwise ship as-is (it is the off-ramp done right). (G.2 #1)
- [taste] Placeholder-voice rule + double-space layout hacks ("Short  ~10 min" SessionSettingsSheet.swift:78-80, "Settle in  ·  N  →" HomeDashboardView.swift:658) → HStack spacing, not string spaces. (G.5 #11)

**Count: 29 net-new punch items (P0 3 · P1 14 · P2 12; [system fix] 23 · [taste] 6).** Directions that yielded explicit nulls after honest search: no one-shot durations > cinematic outside ceremonies, no competing same-property animations, no non-0.96 press scales (G.4); no `symbolRenderingMode` variance (G.5); Settings destructive confirms, Pulse uncharted beats, DiscussionCardView, ResourcesOverlayView, Play-vs-Home joiner banners, and CompositionConfirmCard inspected with no finding.

---

## Orchestrator Verification Log

**[VERIFIED: Subagent G — Pass 2 Gap & Edge Case Audit]** Findings are additive — spot-checked against Pass 1 and none duplicate an existing item; adjacency to Pass 1 items is stated explicitly where present (e.g. scrim sites folded into the existing `scrimHeavy` item, EventEntryEditor CTA folded into the consolidation item). All six mandated directions produced results with file:line specificity, and honest nulls are recorded (no >cinematic one-shots, no competing same-property animations, five inspected states with no finding). Materially new discoveries: a shipped stub sheet reachable in production (PulseInfoSheet), a relative-date bug that fabricates recency ("yesterday" for any age ≥24h), the session reflection being inoperable under VoiceOver, four surfaces absent from the Phase 1 list (including the principle-critical single-user off-ramp), and the laundered-constant class of token violations grep structurally cannot catch. All nine flagged-vague Phase 8 items were concretized into executable form. 29 net-new punch items, each tagged and ranked. Output accepted. Pass 2 complete.

---

## Final Orchestration Summary

**Run:** Autonomous execution of the 2026-07-08 Vayl UI Design Review (Fable orchestrating; the model was switched to Opus 4.8 during Pass 2 finalization). The review prompt file itself was absent from this fresh clone (docs/ is gitignored as of 2026-07-07 and it was not recoverable from git history or Notion), so the review was reconstructed and executed from the detailed execution plan supplied with the scheduled task. `docs/mockups/` and `docs/prototypes/` were likewise unavailable; the entire review is grounded in code + CLAUDE.md contracts, and "mockup-parity" comment claims were treated as intent, not evidence.

**Completed — all seven subagents verified, zero blocked:**
- **A (Phases 1–2):** 15 token files summarized, contract rubric distilled, 20 surfaces + full state inventory with explicit MISSING calls. VERIFIED.
- **B (Phase 3):** 190 view-bearing files swept across 12 compliance categories; iOS 26 API hygiene clean; live light-mode infrastructure and 64 tap-contract-at-risk sites surfaced. VERIFIED.
- **C (Phase 4):** all 20 surfaces critiqued, every finding a concrete before→after with a named token; product-principle and data-honesty catches included. VERIFIED.
- **D (Phase 5):** six coherence axes + five ranked system fixes, root-cause first (the `display(n)` typography loophole). VERIFIED.
- **E (Phase 6):** four accessibility categories with computed WCAG ratios; three severe catches (VoiceOver cannot drive a Card Session; VoiceOver reads paywalled desires; 1.76:1 text on live controls). VERIFIED.
- **F (Phase 8):** 102 punch items tagged/ranked/grouped, top-5 leverage summary, 18-entry leaving-as-is list, count check. VERIFIED.
- **G (Pass 2):** 29 net-new additive items, four missed surfaces, laundered-constant violation class, nine Phase 8 items concretized. VERIFIED.

**Totals:** 131 punch items (Pass 1: 102 + Pass 2: 29). Combined priority tally — **P0: 22 · P1: 81 · P2: 28**. Clean category with zero findings: iOS 26 banned APIs.

**Needs human review (decisions the review deliberately did not make):**
1. **Token-level light-mode infrastructure** (ThemeManager, AppTheme, AppColors.dynamic `light:` params, AppElevation/ThemeModifiers `colorScheme` branches) — hard-purge per the V1 dark-only contract vs retain for future Dawn mode. Feature/design-layer light *branches* are punch-listed for removal regardless; the token infra is the open question.
2. **Orphaned surfaces** — MeCardSheet, MeCardCompact, PrismView, SyncMatch (zero external call sites): land or delete.
3. **FEEL-GATE annotated device-physics constants** — laundered but author-sanctioned; only sweep if the human de-ratifies FEEL-GATE.
4. **Dead-CTA product calls** — the Learn quiz runner and ResearchDatabaseView filter engine are missing features, not just UI bugs; the punch list gates the dead affordances pending the product decision to build them.

**Top 5 highest-leverage refinements (cross-surface, from the merged Pass 1/2 view):** (1) close the `AppFonts.display(n)`/`body(n)` escape hatch with ~6 named tokens + a lint rule; (2) lift the contrast floor at the token level in AppColors and retire `textMuted` from live UI; (3) one motion rule — named tokens through the staples, no token arithmetic, `ambientDwell` for auto-advance; (4) one `VaylCloseButton` + the icon-chrome labeling sweep (fixes coherence, Dynamic Type, and VoiceOver at once); (5) write the spectrum discipline rule into AppColors (min-size, link-token, hue semantics).
